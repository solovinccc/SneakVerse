...
function UserView() {
  ...
  const fetchAllShoes = async () => {
    const shoesRes = await fetch(`http://localhost:8081/api/shoes`, {
  ...
  const fetchAvgPrice = async () => {
    setAvgLoading(true);
    try {
      const res = await fetch("http://localhost:8081/api/shoes/avg-price", {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  useEffect(() => {
    const t = setTimeout(async () => {
      ...
      try {
        ...
        const res = await fetch(
          `http://localhost:8081/api/shoes/by-brand-name?brand=${encodeURIComponent(q)}`,
          { headers: { Authorization: `Bearer ${token}` } },
        );
        if (!res.ok)
